...
class ModuleDataController extends Controller
{
    ...
    private function moduleConfig(string $module): array
    {
        $configs = [
            ...
            'sessions' => [
                ...
                'title' => 'Sesi',
                'columns' => ['id', 'name', 'start_at', 'end_at'],
            ],
        ];
    ...
    private function buildSessionDateTime(string $startTime, string $endTime): array
    {
        $timezone = (string) config('app.timezone', 'UTC');
        $startAt = Carbon::createFromFormat('Y-m-d H:i', '2000-01-01 ' . $startTime, $timezone)->second(0);
        $endAt = Carbon::createFromFormat('Y-m-d H:i', '2000-01-01 ' . $endTime, $timezone)->second(0);
    ...
    private function applySearch($query, string $module, string $q): void
    {
        ...
        $query->where(function ($inner) use ($module, $like) {
            match ($module) {
                ...
                'sessions' => $inner->where('name', 'like', $like)->orWhere('status', 'like', $like)->orWhere('id', 'like', $like),
                default => null,
            };
        });
    }
