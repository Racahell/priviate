                ...
                <ul class="sidebar-menu">
                    @php
                      ...
                      if (auth()->check() && \Illuminate\Support\Facades\Schema::hasTable('menu_items') && \Illuminate\Support\Facades\Schema::hasTable('menu_permissions')) {
                          ...
                          if (in_array($role, ['admin', 'superadmin'], true) && Route::has('admin.sessions')) {
                              $hasSessionsMenu = $dynamicMenus->contains(fn ($m) => (string) ($m->route_name ?? '') === 'admin.sessions');
                              if (!$hasSessionsMenu) {
                                  $dynamicMenus->push((object) ['route_name' => 'admin.sessions', 'label' => 'Sesi']);
                              }
                          }
